...
/*
** Update a segment's page pointers (iFirst, iLastPg, iRoot) after a block
** has been physically moved from iFrom to iTo. This does NOT copy block data.
*/
void lsmFsUpdateSegPages(FileSystem *pFS, Segment *pSeg, int iTo, int iFrom){
  fsMovePage(pFS, iTo, iFrom, &pSeg->iFirst);
  fsMovePage(pFS, iTo, iFrom, &pSeg->iLastPg);
  fsMovePage(pFS, iTo, iFrom, &pSeg->iRoot);
}
