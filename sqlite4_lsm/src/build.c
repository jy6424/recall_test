...
#ifndef SQLITE4_OMIT_VIRTUALTABLE
  if( p==0 ){
    /* If the table was not found, check for an eponymous virtual table */
    Module *pMod = (Module*)sqlite4HashFind(&pParse->db->aModule, zName,
                                            sqlite4Strlen30(zName));
    if( pMod && sqlite4VtabEponymousTableInit(pParse, pMod) ){
      p = pMod->pEpoTab;
    }
  }
#endif
...
static void destroyTable(Parse *pParse, Table *pTab){
  ...
  for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){
    if( IsVectorIndex(pIdx) ){
      ...
      vectorIndexDrop(pParse->db, pParse->db->aDb[iDb].zName, pIdx->zName);
    }
  }
#endif
  for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){
...
Index *sqlite4CreateIndex(
  ...
  */
#ifndef SQLITE4_OMIT_VECTOR
  /* Only check for vector indexes on explicit CREATE INDEX (pName!=0),
  ** not on implicit PK/UNIQUE constraints created during CREATE TABLE */
  if( pName ){
    int vectorIdxRc = vectorIndexCreate(pParse, pIndex, db->aDb[iDb].zName, 0, pList);
    if( vectorIdxRc < 0 ){
      goto exit_create_index;
    }
    if( vectorIdxRc >= 1 ){
      /* Vector index: set aiColumn to the embedding column extracted from
      ** the libsql_vector_idx(column) expression, skip normal column scan */
      pIndex->idxIsVector = 1;
      pIndex->aiColumn[0] = 0;
      if( pList->nExpr >= 1 && pList->a[0].pExpr ){
        ExprList *pArgsList = pList->a[0].pExpr->x.pList;
        if( pArgsList && pArgsList->nExpr >= 1 ){
          Expr *pArg = pArgsList->a[0].pExpr;
          if( pArg->op == TK_COLUMN ){
            pIndex->aiColumn[0] = pArg->iColumn;
          }else if( pArg->op == TK_ID ){
            /* Resolve column name to index manually */
            int k;
            for(k=0; k<pTab->nCol; k++){
              if( sqlite4_stricmp(pTab->aCol[k].zName, pArg->u.zToken)==0 ){
                pIndex->aiColumn[0] = k;
                break;
              }
            }
      ...
      pIndex->azColl[0] = db->pDfltColl->zName;
      pIndex->aSortOrder[0] = 0;
      sqlite4DefaultRowEst(pIndex);
      goto vector_index_columns_done;
    }
  }
#endif
...
void sqlite4DropIndex(Parse *pParse, SrcList *pName, int ifExists){
  ...
  if( IsVectorIndex(pIndex) ){
    vectorIndexDrop(db, db->aDb[iDb].zName, pIndex->zName);
  }
#endif
...
void sqlite4SrcListDelete(sqlite4 *db, SrcList *pList){
  ...
  for(pItem=pList->a, i=0; i<pList->nSrc; i++, pItem++){
    ...
    sqlite4ExprListDelete(db, pItem->pFuncArg);
  }
  sqlite4DbFree(db, pList);
}
...
*/
void sqlite4SrcListFuncArgs(Parse *pParse, SrcList *p, ExprList *pList){
  if( p ){
    SrcListItem *pItem = &p->a[p->nSrc-1];
    assert( pItem->notIndexed==0 );
    assert( pItem->zIndex==0 );
    assert( pItem->isTabFunc==0 );
    pItem->pFuncArg = pList;
    pItem->isTabFunc = 1;
  }else{
    sqlite4ExprListDelete(pParse->db, pList);
  }
...
KeyInfo *sqlite4IndexKeyinfo(Parse *pParse, Index *pIdx){
  ...
  if( pKey ){
    ...
    /* Store index and database names for vector index support */
    {
      int iDbIdx = sqlite4SchemaToIndex(db, pIdx->pSchema);
      if( 0 <= iDbIdx && iDbIdx < db->nDb ){
        pKey->zDbSName = sqlite4DbStrDup(db, db->aDb[iDbIdx].zName);
      }
      pKey->zIndexName = sqlite4DbStrDup(db, pIdx->zName);
    }
  }
