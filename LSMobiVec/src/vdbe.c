...
/* Return true if the cursor is a vector index cursor. */
#ifndef SQLITE4_OMIT_VECTOR
#define isVectorCursor(x) ((x)->pVecIdx!=0)
#else
#define isVectorCursor(x) 0
#endif

static u32 saturating_add(u32 lhs, u32 rhs) {
    u64 sum = (u64)lhs + (u64)rhs;
    return (u32)(sum > 0xFFFFFFFF ? 0xFFFFFFFF : sum);
}

void libsql_inc_row_read(Vdbe *p, int count) {
    p->aLibsqlCounter[0] = saturating_add(p->aLibsqlCounter[0], count);
}

void libsql_inc_row_written(Vdbe *p, int count) {
    p->aLibsqlCounter[1] = saturating_add(p->aLibsqlCounter[1], count);
}
...
**
** Open a vector index cursor. P1 is the cursor number. P2 is the root page.
** P3 is the database index. P4 is the KeyInfo. P5 flags (OPFLAG_FORDELETE
** means clear the index first).
*/
...
case OP_Found: {        /* jump, in3 */
  ...
  assert( pC!=0 );
#ifndef SQLITE4_OMIT_VECTOR
  /* Vector index cursors have no KV cursor — treat as "not found" */
...
case OP_Insert: {
  ...
  if( isVectorCursor(pC) ){
    ...
    ** key (integer key) and vector from P2 data. */
    i64 rowid = 0;
...
#ifndef SQLITE4_OMIT_VECTOR
  if( isVectorCursor(pC) ){
    /* For vector index cursors, extract rowid and call vectorIndexDelete */
    i64 rowid = 0;
    if( pKey->flags & MEM_Int ){
      rowid = sqlite4VdbeIntValue(pKey);
    }else if( pKey->flags & MEM_Blob ){
      /* Try to decode the rowid from the key blob */
      sqlite4_num num;
      if( sqlite4VdbeDecodeNumericKey((const KVByteArray*)pKey->z, pKey->n, &num)==SQLITE4_OK ){
        rowid = sqlite4_num_to_int64(num, 0);
      }
    }
    rc = vectorIndexDelete(pC->pVecIdx, rowid, &p->zErrMsg);
    if( rc ) goto abort_due_to_error;
    pC->rowChnged = 1;
    break;
  }
...
case OP_VFilter: {   /* jump */
  ...
  nArg = sqlite4_num_to_int32(pArgc->u.num, 0);
  iQuery = sqlite4_num_to_int32(pQuery->u.num, 0);
