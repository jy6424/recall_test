...
def prepare_sql(sql_path):
    """Strip PRAGMA lines (not needed — page size is compile-time for both)."""
    lines = []
...
def run_one_config(label, shell, compact_bin, insert_sql_path, query_sql_path,
    ...
    insert_sql = prepare_sql(insert_sql_path)
    t0 = time.time()
    ...
    query_sql = prepare_sql(query_sql_path)
    t0 = time.time()
