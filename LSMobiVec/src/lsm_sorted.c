...
/* Auto-compaction timing globals */
...
static u8 *pageGetKey(
  ...
  eType = *pKey++;
  pKey += lsmVarintGet64(pKey, &dummy);
...
static int segmentPtrLoadPage(
  ...
  Pgno iNew                      /* Page number of new page */
){
...
static int mergeWorkerPageOffset(u8 *aData, int nData){
  ...
  iOff += lsmVarintGet64(&aData[iOff], &dummy);
  if( eType==0 ){
    iOff += lsmVarintGet64(&aData[iOff], &dummy);
  }else{
...
static int sortedSelectLevel(lsm_db *pDb, int nMerge, Level **ppOut){
  ...
  ** with the largest number of right-hand segments. Work on it. */
  ...
  if( pBest ){
    if( pBest->nRight==0 ){
      /* Cap merge batch size at nMerge so compaction produces the same
      ** hierarchical level structure as incremental autowork. */
      int nActual = (nMerge>1 && nBest>nMerge) ? nMerge : nBest;
      rc = sortedMergeSetup(pDb, pBest, nActual, ppOut);
    }else{
      *ppOut = pBest;
    }
  }
...
static int sortedMoveBlock(lsm_db *pDb, int *pnWrite){
  ...
  Level *pIter;                   /* Iterator for levels */
  int iFrom;                      /* Block to move */
  int iTo;                        /* Destination to move block to */
  int rc;                         /* Return code */
  int bFirst = 1;                 /* True for the first segment processed */
  ...
  ** 1 is tricky as it is smaller than the other blocks.  */
  ...
  /* Move the block data using the first segment, then update page pointers
  ** for all other segments. lsmFsMoveBlock copies the block and updates one
  ** segment's pointers. lsmFsUpdateSegPages updates pointers only.  */
  rc = lsmFsMoveBlock(pDb->pFS, &pLvl->lhs, iTo, iFrom);
  if( rc==LSM_OK ){
    /* Update page pointers for all other segments across all levels */
    for(pIter=pLvl; pIter; pIter=pIter->pNext){
      int i;
      if( bFirst ){
        bFirst = 0;  /* Skip pLvl->lhs, already done by lsmFsMoveBlock */
      }else{
        lsmFsUpdateSegPages(pDb->pFS, &pIter->lhs, iTo, iFrom);
      }
      for(i=0; i<pIter->nRight; i++){
        lsmFsUpdateSegPages(pDb->pFS, &pIter->aRhs[i], iTo, iFrom);
      }
    }
  }
  ...
  if( rc==LSM_OK ){
    ...
    if( rc==LSM_OK ){
      ...
      *pnWrite = lsmFsBlockSize(pDb->pFS) / lsmFsPageSize(pDb->pFS);
      ...
      /* Set pRedirect on ALL segments across all levels */
      for(pIter=pLvl; pIter; pIter=pIter->pNext){
        int j;
        pIter->lhs.pRedirect = &p->redirect;
        for(j=0; j<pIter->nRight; j++){
          pIter->aRhs[j].pRedirect = &p->redirect;
        }
      }
    }
  }
...
static int doLsmSingleWork(
  ...
  int bFlushOnly                  /* If true, flush only — no merge work */
){
...
static int doLsmWork(lsm_db *pDb, int nMerge, int nPage, int bFlushOnly, int *pnWrite){
  int rc = LSM_OK;                /* Return code */
  ...
  if( nPage!=0 ){
    ...
    do {
      ...
      bCkpt = 0;
      rc = doLsmSingleWork(pDb, 0, nMerge, nReq, &nThis, &bCkpt, bFlushOnly);
      nWrite += nThis;
      if( rc==LSM_OK && bCkpt ){
...
int lsm_work(lsm_db *pDb, int nMerge, int nKB, int *pnWrite){
  ...
  ** transaction. Return LSM_MISUSE if an application attempts this.  */
  ...
  rc = doLsmWork(pDb, nMerge, nPage, 0, &nWrite);
...
/*
** Reclaim free space by relocating blocks from the end of the database
...
int lsm_reclaim(lsm_db *pDb, int nKB, int *pnWrite){
  ...
  ** while other levels still need it, causing SIGBUS on truncated blocks. */
  ...
  while( rc==LSM_OK ){
    int nDone = 0;
    rc = sortedMoveBlock(pDb, &nDone);
    if( rc==LSM_OK && nDone==0 ) break;
    nWrite += nDone;
  }

  if( rc==LSM_OK && nWrite>0 ){
    Snapshot *pWorker = pDb->pWorker;
    Freelist *pFL = &pWorker->freelist;
    int nBlkBefore = pWorker->nBlock;
    int nNewBlock;
    int i, j;
    MoveBlockCtx sCtx;

    /* Shrink nBlock: walk the FULL freelist (disk + memory) in reverse
    ** to find the highest data block. */
    sCtx.iSeen = pWorker->nBlock + 1;
    sCtx.iFrom = 0;
    rc = lsmWalkFreelist(pDb, 1, moveBlockCb, &sCtx);
    if( rc==LSM_OK ){
      if( sCtx.iFrom>0 ){
        nNewBlock = sCtx.iFrom;
      }else if( sCtx.iSeen <= pWorker->nBlock ){
        nNewBlock = sCtx.iSeen - 1;
      }else{
      ...
      ** reallocate blocks that already contain live moved data. */
      ...
      pFL->nEntry = j;
      pWorker->nBlock = nNewBlock;
      fprintf(stderr, "  reclaim: nBlock %d -> %d, redirect.n=%d\n",
          nBlkBefore, nNewBlock, pWorker->redirect.n);
    }
...
reclaim_out:
  if( pnWrite ){
    *pnWrite = (int)(((i64)nWrite * nPgsz + 1023) / 1024);
  }
  return rc;
}
...
int lsmSortedAutoWork(
  ...
  ** work is achieved by writing one page (~4KB) of merged data.  */
  ...
  if( nDepth>0 ){
    ...
    struct timespec aw_start, aw_end;
...
#endif
    assert( nRemaining>=0 );
    clock_gettime(CLOCK_MONOTONIC, &aw_start);
    rc = doLsmWork(pDb, pDb->nMerge, nRemaining, !pDb->bAutowork, 0);
    clock_gettime(CLOCK_MONOTONIC, &aw_end);
    g_autoworkTotalMs += autowork_time_ms(&aw_start, &aw_end);
    g_autoworkCalls++;
...
int lsmFlushTreeToDisk(lsm_db *pDb){
  ...
  rc = lsmBeginWork(pDb);
  while( rc==LSM_OK && pDb->bAutowork && sortedDbIsFull(pDb) ){
...
void sortedDumpPage(lsm_db *pDb, Segment *pRun, Page *pPg, int bVals){
  ...
  lsmLogMessage(pDb, LSM_OK, "      Page %lld: %s", lsmFsPageNumber(pPg), s.z);
  lsmStringClear(&s);
