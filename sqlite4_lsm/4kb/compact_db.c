...
int main(int argc, char **argv){
  ...
  fprintf(stderr, "Phase 1: merge (nMerge=4)\n");
  do {
    rc = lsm_work(pDb, 4, 4096, &nWritten);
    if( rc != 0 ){
      fprintf(stderr, "lsm_work(nMerge=4) failed: %d\n", rc);
      break;
    }
    nTotal += nWritten;
    if( nWritten > 0 ){
  ...
  /* Show DB structure after Phase 1 */
  {
    char *zInfo = 0;
    lsm_info(pDb, LSM_INFO_DB_STRUCTURE, &zInfo);
    fprintf(stderr, "After phase 1: %s\n", zInfo ? zInfo : "(null)");
    lsm_free(pEnv, zInfo);
  }

  /* Phase 2: Compact to single segment with nMerge=1.
  ** Required before lsm_reclaim can safely move blocks. */
  if( rc == 0 ){
    fprintf(stderr, "Phase 2: compact to single segment (nMerge=1)\n");
    nTotal = 0;
    do {
  ...
  /* Show DB structure after Phase 2 */
  {
    char *zInfo = 0;
    lsm_info(pDb, LSM_INFO_DB_STRUCTURE, &zInfo);
    fprintf(stderr, "After phase 2: %s\n", zInfo ? zInfo : "(null)");
    lsm_free(pEnv, zInfo);
  }

  /* Reclaim free space (shrink file) */
  if( rc == 0 ){
    fprintf(stderr, "Phase 3: reclaim free space\n");
    nTotal = 0;
    do {
    ...
    fprintf(stderr, "Phase 3 done. Total: %d KB relocated.\n", nTotal);
  }
